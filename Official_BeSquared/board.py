...
class Board:
    ...
    def check_collision(self, mouse_pos):
        if self.is_game_over:
            if self.retry.collidepoint(mouse_pos):
                self.score = 0
                self.init_game_board()
                self.seconds_left = self.time_of_game
                self.is_game_over = False
        ...
        for cur_row in range(self.rows):
            for cur_col in range(self.cols):
                ...
                if self.jewels[cur_row][cur_col].check_collision(mouse_pos):
    ...
    def draw(self):
        ...
        lbl_high_score = self.score_font.render("--Highscores-- ", 1, (255, 255, 255))
        self.screen.blit(lbl_high_score, (20, self.height/4))
        ...
        if self.is_game_over:
            lbl_game_over = self.scoreFont.render("Game Over", 1, (255, 255, 255))
    ...
    def check_3_in_a_row_horizontal(self, row, col):
        ...
        try:
            ...
            #do nothing here right now, this happens when you try to check jewels that havent been created yet
            temp = 0
    ...
    def check_high_score(self):
        ...
        if len(self.scores) < 3 and not has_inserted_score:
            # only do this if you havent inserted the score yet
            print("Congrats you have a new high score!!")
            new_name = input("Enter your name: ")
